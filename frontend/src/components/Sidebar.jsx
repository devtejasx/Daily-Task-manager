    ...
    >
      ...
      <div className="hidden lg:flex items-center gap-3 px-5 xl:px-6 h-20 border-b border-violet-500/15">
        ...
        <div className="hidden xl:block">
          ...
          <p className="text-[9px] tracking-[0.3em] text-violet-300/70 font-semibold">
            COMMAND CENTER
          </p>
        </div>
      </div>

      <nav className="flex-1 flex lg:flex-col justify-around lg:justify-start gap-1 p-2 lg:p-3 lg:mt-4 overflow-x-auto lg:overflow-x-visible">
        {NAV_ITEMS.map(({ id, path, label, icon: Icon }) => (
          <NavLink
            key={id}
            to={path}
            className={({ isActive }) =>
              `relative flex flex-col lg:flex-row items-center lg:gap-3 gap-1 px-3 py-2.5 lg:px-4 lg:py-3 rounded-xl
              transition-colors duration-300 group flex-1 lg:flex-none justify-center lg:justify-start
              focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/70
              ${isActive ? "text-cyan-200" : "text-slate-400 hover:text-violet-200"}`
            }
          >
            {({ isActive }) => (
              <>
      ...
      <div className="hidden xl:block px-6 pb-6 text-[10px] text-slate-500 tracking-widest">
        SYSTEM v0.2 · ONLINE
        <span
          className="ml-2 inline-block w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_#10b981] animate-pulse"
          aria-hidden
        />
      </div>
    </motion.aside>
